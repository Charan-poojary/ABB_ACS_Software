...
const DriverMaster = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5213/api/DriverDetails/${editedEmployeeMaster.empid}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleSearch = async () => {
        try {
            setLoading(true);
            console.log("kkk",searchParams);
            const query = new URLSearchParams(searchParams).toString();
            const response = await fetch(`http://localhost:5213/api/DriverDetails/Search?${query}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    return (
        <>
            <div className="container employeeContainer mt-2 justify-content-center align-items-start">
                <div className='card'>
                    <h1 className="mb-4 ms-2">Driver Master</h1>
