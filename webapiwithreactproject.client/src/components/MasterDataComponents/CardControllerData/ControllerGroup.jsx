...
const ControllerGroup = () => {
    ...
    const fetchSectionMasters = async () => {
        try {
            setLoading(true);
            const response = await fetch('http://localhost:5213/api/Masterdgs');
            if (!response.ok) {
    ...
    const handleAddClick = async () => {
        ...
        try {
            ...
            const checkResponse = await fetch(`http://localhost:5213/api/Masterdgs/CheckAccessGroup/${newSectionMaster.dgname}`);
            if (!checkResponse.ok) {
                throw new Error('Network response was not ok');
            }
    ...
    const handleSave = async () => {
        if (!editedSectionMaster.dgname) {
            setError("Fields cannot be empty");
            return;
        }

        try {
            const response = await fetch(`http://localhost:5213/api/Masterdgs/${editedSectionMaster.dgid}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    dgid: editedSectionMaster.dgid,
                    dgname: editedSectionMaster.dgname,
    ...
    const handleSelectedDelete = async () => {
        try {
            await Promise.all(selectedRows.map(async (srno) => {
                const response = await fetch(`http://localhost:5213/api/Masterdgs/${srno}`, {
                    method: 'DELETE',
                });
    ...
    return (
        <>
            <div className="container employeeContainer mt-2 justify-content-center align-items-start">
                <div className='card'>
                    <h1 className="mb-4 ms-2">Controller Groups Master</h1>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    sectionMasters.map(sectionMaster => (
                                        <tr key={sectionMaster.dgid}>
                                            <td>
                                                <input
                                                    type="checkbox"
                                                    onChange={() => handleCheckboxChange(sectionMaster.dgid)}
                                                    checked={selectedRows.includes(sectionMaster.dgid)}
                                                />
                                            </td>
                                            <td>{sectionMaster.dgname}</td>
                                            <td>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-header">
                            <h5 className="modal-title" id="addModalLabel">Add Controller Group</h5>
                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div className="modal-body">
                            <form>
                                <div className="mb-3">
                                    <label htmlFor="dgname" className="form-label2">Controller Group Name</label>
                                    <input
                                        type="text"
                                        className="form-control2"
                                        id="dgname"
                                        name="dgname"
                                        value={newSectionMaster.dgname}
            ...
            <div className="modal fade" id="editModal" tabIndex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        {successMessage && <div className="alert alert-success">{successMessage}</div>}
                        <div className="modal-header">
                            <h5 className="modal-title" id="editModalLabel">Edit Controller Group</h5>
                            <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div className="modal-body">
                            <form>
                                <div className="mb-3">
                                    ...
                                    <input
                                        ...
                                        id="dgname"
                                        name="dgname"
                                        value={editedSectionMaster.dgname}
